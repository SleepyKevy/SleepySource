...
internal sealed class AlertService
{
    ...
    private static AlertStyle NormalizeStyle(AlertStyle s, AlertStyle f, int cw, int ch)
    {
        ...
        s.BackgroundColor = AppUtil.NormalizeColor(s.BackgroundColor, f.BackgroundColor); s.BackgroundOpacity = AppUtil.Clamp(s.BackgroundOpacity, 0, 100); s.AccentColor = AppUtil.NormalizeColor(s.AccentColor, f.AccentColor); s.Radius = AppUtil.Clamp(s.Radius, 0, 120); s.BorderWidth = AppUtil.Clamp(s.BorderWidth, 0, 20); s.SoundVolume = AppUtil.Clamp(s.SoundVolume, 0, 100); s.SoundDelayMS = AppUtil.Clamp(s.SoundDelayMS, 0, 10000); s.VisualFile = string.IsNullOrWhiteSpace(s.VisualFile) ? null! : Path.GetFileName(s.VisualFile.Trim()); s.SoundFile = string.IsNullOrWhiteSpace(s.SoundFile) ? null! : Path.GetFileName(s.SoundFile.Trim()); s.Animation = s.EnterAnimation; s.TextColor = s.TitleColor;
        return s;
    }
