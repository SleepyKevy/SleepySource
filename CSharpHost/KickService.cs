...
internal sealed class KickUserAuthService
{
    ...
    private async Task<(string Access,string Refresh,string Type,string Scope,long Expires)> RequestUserTokenAsync(Dictionary<string,string> form,CancellationToken ct){using var http=new HttpClient{Timeout=TimeSpan.FromSeconds(15)};using var req=new HttpRequestMessage(HttpMethod.Post,KickService.OAuthBase+"/oauth/token"){Content=new FormUrlEncodedContent(form)};req.Headers.UserAgent.ParseAdd("SleepySource/1.1");using var resp=await http.SendAsync(req,ct);var data=await resp.Content.ReadAsByteArrayAsync(ct);if(!resp.IsSuccessStatusCode)throw new InvalidOperationException("Kick OAuth returned "+resp.StatusCode+": "+Encoding.UTF8.GetString(data).Trim());using var doc=JsonDocument.Parse(data);var r=doc.RootElement;var a=r.TryGetProperty("access_token",out var at)?at.GetString()?.Trim()??"":"";if(a.Length==0)throw new InvalidOperationException("Kick OAuth did not return a user access token");var refh=r.TryGetProperty("refresh_token",out var rt)?rt.GetString()?.Trim()??"":"";var typ=r.TryGetProperty("token_type",out var ty)?ty.GetString()?.Trim()??"":"";var sc=r.TryGetProperty("scope",out var s)?s.GetString()?.Trim()??"":"";long ex=3600;if(r.TryGetProperty("expires_in",out var ei)){if(ei.ValueKind==JsonValueKind.Number)ei.TryGetInt64(out ex);else long.TryParse(ei.GetString(),out ex);}return(a,refh,typ,sc,Math.Max(60,ex));}
